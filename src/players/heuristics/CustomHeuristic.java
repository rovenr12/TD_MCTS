...
public class CustomHeuristic extends StateHeuristic {
    ...
    public static class BoardStats
    {
        ...
        BoardStats(GameState gs) {
            ...
            for(int i=myPosition.x-2; i<=myPosition.x+2; i++){
                for(int j=myPosition.y-2; i<=myPosition.y+2; i++){
                    if (i>=0 && j>=0 && i<11 && j<11){
                        for (int k=0; k<gs.getEnemies().length;k++){
                            if (gs.getBoard()[i][j] == gs.getEnemies()[k]){
                                nearbyEnemy++;
                            }
                        }
                    }
        ...
        double score(BoardStats futureState)
        {
            ...
            return (diffEnemies / 3.0) * FACTOR_ENEMY + diffTeammates * FACTOR_TEAM + (diffWoods / maxWoods) * FACTOR_WOODS
                    + diffCanKick * FACTOR_CANKCIK + (diffBlastStrength / maxBlastStrength) * FACTOR_BLAST + futureState.nearbyEnemy * 0.1;
        }
    }
}
