package GroupK.OpponentEnhacement;
...
public class SingleTreeNode1
{
    ...
    SingleTreeNode1(MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNode1(MCTSParams p, SingleTreeNode1 parent, int childIdx, Random rnd, int num_actions,
                            Types.ACTIONS[] actions, int fmCallsCount, StateHeuristic sh) {
        this.params = p;
        this.fmCallsCount = fmCallsCount;
        this.parent = parent;
//        if (parent != null && parent.childIdx == -1){
//            System.out.println(parent.childIdx);
//        }
        this.m_rnd = rnd;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            SingleTreeNode1 selected = treePolicy(state);
    ...
    private void roll(GameState gs, Types.ACTIONS act)
    {
        ...
        for(int i = 0; i < nPlayers; ++i)
        {
            if(playerId == i)
            {
                ...
                int[] opponentPosition = opponetPosition(i, gs);
                actionsAll[i] = Types.ACTIONS.all().get(safeRandomAction(opponentPosition[0], opponentPosition[1], gs));
            }
        }
    ...
    private int[] opponetPosition(int agentIndex, GameState state){
    ...
    // Opponent Improvement
    private int safeRandomAction(int x, int y, GameState gs) {
    ...
    // Opponent Improvement
    private boolean safeAction(int posX, int posY, Vector2d dir, GameState gs){
